...
describe('Club Repository Test.', () => {
	...
	let repo: Mock<Repository<WorldCup>>;
	let wcRepo: Mock<Prisma.WorldCupDelegate<DefaultArgs>>;

	beforeEach(() => {
		repo = new Mock<Repository<WorldCup>>();
	...
	it('Find one With Participants.', async () => {
		//Arrange
		const uuid = 'c46f7322-9045-11ed-923d-0242ac180003';
		repo.setup((m) => m.findOne(It.IsAny())).returnsAsync(new WorldCup());
	...
	it('Find all.', async () => {
		//Arrange
		repo.setup((m) => m.find(It.IsAny())).returnsAsync([new WorldCup()]);
	...
	it('Call save.', async () => {
		//Arrange
		const worldCupEntity = new WorldCup();
		worldCupEntity.id = 1;
		...
		repo.setup((m) => m.save(It.IsAny())).returnsAsync(new WorldCup());
